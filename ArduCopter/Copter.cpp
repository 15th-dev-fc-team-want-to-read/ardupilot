...
bool Copter::set_target_rate_and_thrust(float roll_rate_rads, float pitch_rate_rads, float yaw_rate_rads, float thrust)
{
    // exit if vehicle is not in Guided mode or Auto-Guided mode
    if (!flightmode->in_guided_mode()) {
        return false;
    ...
    Quaternion q;
    q.zero();
    Vector3f rate(roll_rate_rads, pitch_rate_rads, yaw_rate_rads);
